module.exports = {
  quiz: [
    ...
    { q: 'The average of 5, 10 and 15 is…', a: ['10', '30', '15', '5'], c: 0, why: '(5 + 10 + 15) ÷ 3 = 10.' },
...
`,
    js: function (el, api) {
      ...
      function render() {
        ...
        if (step >= 3) h += '<div class="ms-step' + (step >= 4 ? ' done' : '') + '"><p><b>4 · Conclude:</b> Which conclusion is supported by this data?</p><div class="row" id="ms-conc">' + [['b', 'You scored 60 points once, so you’re a superstar'], ['a', 'You scored steadily, around 14 points a game'], ['c', 'Your new shoes caused a good season']].map(function (c) { return '<button type="button" class="btn small" data-v="' + c[0] + '" ' + (step >= 4 ? 'disabled' : '') + '>' + c[1] + '</button>'; }).join('') + '</div><p class="feedback" id="ms-f4" aria-live="polite">' + (step >= 4 ? '✅ Supported by clean data, with no invented causes. Report complete!' : '') + '</p></div>';
        el.innerHTML = h;
